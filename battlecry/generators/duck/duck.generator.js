...
export default class DuckGenerator extends Generator {
  ...
  init() {
    ...
    if (configFile.exists)
      return log.warn(
        `Modular ducks have already been initiated. Please check the ${configFile.path} file`
      )
  ...
  addActionsToDuck() {
    ...
    this.actions.forEach(action => {
      file.after(
        "// Actions",
        `const __NA_ME__ = '${casex(this.args.name, "na-me")}/__NA-ME__';`,
        action
      )

      file.after(
        "switch (action.type) {",
        ["    case __NA_ME__:", "      // Perform action", "      return state;"],
        action
      )
  ...
  addDuckToConfig() {
    ...
    file
      .afterLast(
        "import ",
        "import __naMe__ from './modules/__naMe__'",
        this.args.name
      )
